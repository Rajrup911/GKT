...
class KernelAttention(nn.Module):
    ...
    def forward(self, q, k, v, skip=None, mask=None):
        ...
        if og_code:
            ...
            _, _, H, W = q.shape
            b = q.shape[0] // num_images
            num_points = k.shape[-2]
            # Move feature dim to last for multi-head proj
            # (b, n, k, d)
            q = rearrange(q, 'n d H W -> n (H W) d')
        ...
        if og_code:
            ...
            # Group the head dim with batch dim
            q = rearrange(q, 'n q (m d) -> (m n) q 1 d',
                        m=self.heads, d=self.dim_head)
            k = rearrange(k, 'n q g (m d) -> (m n) q g d',
                        m=self.heads, d=self.dim_head)
            v = rearrange(v, 'n q g (m d) -> m q (n g) d',
                        m=self.heads, d=self.dim_head)

            # Dot product attention along cameras
            dot = self.scale * \
                torch.einsum('n Q c d, n Q K d -> n Q c K', q, k)
            print(dot.shape)
            exit()
            dot = rearrange(dot, '(b n) Q c K -> b Q (n c K)', b=b)
        ...
        if mask is not None:
            if og_code:
                ...
                mask = mask.repeat(self.heads, 1, 1, num_points)
                mask = rearrange(mask, 'h n Q g -> h Q (n g)')
                print(dot.shape, mask.shape)
                exit()
                dot[~mask] = -10**9
...
def bev2image_sampling(points, I, E, height, width):
    ...
    if og_code:
        ...
        # (k, 3) -> (n, k, 4, 1)
        points = points.view(1, k, 4).repeat(n, 1, 1).unsqueeze(-1)
        # (b, n, 4, 4) * (k, 4)^T
        point2image = (intrin_mat @ E).view(n, 1, 4, 4).repeat(1, k, 1, 1)

    sample_points = (point2image @ points).squeeze(-1).unsqueeze(0)  # (b, n, k, 4)
...
class IndexBEVProjector(nn.Module):
    ...
    def forward(self, bev_grids, images, I, E):
        ...
        if og_code:
            ...
            # [n, k, 9, 2]
            sample_points = sample_points.permute(1, 2, 0, 3) + grid_offsets
...
class GeometryKernelAttention(nn.Module):
    ...
    def forward(
        ...
        if og_code:
            ...
            n, _, _, _ = feature.shape
            b = n // num_images

            # n 3 h w
            pixel = self.image_plane.squeeze(0)
            _, _, h, w = pixel.shape
        ...
        if og_code:
            ...
            img_embed = d_embed - c_embed.view(n, 1, 1, d_embed.shape[-1])
